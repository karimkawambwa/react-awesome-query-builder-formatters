...
import {
  getFieldConfig, getFieldWidgetConfig, getFuncConfig, getOperatorConfig
} from "../utils/configUtils";
import { defaultConjunction } from "../utils/defaultUtils";
import { SqlString } from "../utils/export";
import { completeValue } from "../utils/funcUtils";
import { formatFieldName, getFieldPath, getFieldPathLabels, getWidgetForFieldOp } from "../utils/ruleUtils";
import { defaultValue } from "../utils/stuff";


export const sqlFormat = (tree, config, alias) => {
  return _sqlFormat(tree, config, alias, false);
};

export const _sqlFormat = (tree, config, alias, returnErrors = true) => {
  //meta is mutable
  let meta = {
    errors: []
  };

  const res = formatItem(tree, config, meta, alias);
...
const formatItem = (item, config, meta, alias) => {
  if (!item) return undefined;
  const type = item.get("type");
  const children = item.get("children1");

  if ((type === "group" || type === "rule_group") && children && children.size) {
    return formatGroup(item, config, meta, alias);
  } else if (type === "rule") {
    return formatRule(item, config, meta, alias);
  }
...
const formatGroup = (item, config, meta, alias) => {
  const type = item.get("type");
  ...
  const list = children
    .map((currentChild) => formatItem(currentChild, config, meta, alias))
...
const formatRule = (item, config, meta, alias) => {
  const properties = item.get("properties") || new Map();
  ...
  //format field
  const formattedField = (alias ? `${alias}.` : "") + formatField(meta, config, field);

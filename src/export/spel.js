...
const buildFnToFormatOp = (operator, operatorDefinition) => {
  ...
  if (cardinality == 0) {
    fn = (
      ...
      if (isMethod) return `${field}.${sop}()`
      else return `${field} ${sop}`
    }
    ...
    fn = (
      ...
      if (objectIsFirstArg) return `${values}.${sop}(${field})`
      else if (isMethod) return `${field}.${sop}(${values})`
      else return `${field} ${sop} ${values}`
    }
  }
  return fn
}

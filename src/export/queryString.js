...
const formatGroup = (
  ...
  const groupField =
    isRuleGroup && mode == "array" ? properties.get("field") : null
  const canHaveEmptyChildren = isRuleGroup && mode == "array"
  const not = properties.get("not")
  const list = children
    .map((currentChild) =>
      formatItem(currentChild, config, meta, isForDisplay, groupField)
    )
  ...
  const conjStr = list.size
    ? conjunctionDefinition.formatConj(list, conjunction, not, isForDisplay)
    : null

  let ret
  if (groupField) {
    const aggrArgs = formatRule(
      item,
      config,
      meta,
      isForDisplay,
    ...
    if (aggrArgs) {
      ...
      const args = [conjStr, ...aggrArgs]
      ret = config.settings.formatAggr(...args)
      if (isRev) {
        ret = config.settings.formatReverse(
          ret,
          null,
          null,
          null,
...
const formatField = (
  ...
  if (field) {
    ...
    const fieldParts = Array.isArray(field)
      ? field
      : field.split(fieldSeparator)
    const _fieldKeys = getFieldPath(field, config)
    const fieldPartsLabels = getFieldPathLabels(
      field,
      config,
      cutParentField ? parentField : null
    )
    const fieldFullLabel = fieldPartsLabels
      ? fieldPartsLabels.join(fieldSeparatorDisplay)
      : null
...
const formatFunc = (
  ...
  if (typeof funcConfig.formatFunc === "function") {
    ...
    const args = [formattedArgs, isForDisplay]
    ret = fn(...args)
  } else {

...
const formatGroup = (
  ...
  const hasParentRuleGroup =
    parents.filter((it) => it.get("type") == "rule_group").length > 0
  ...
  const groupFieldName = formatFieldName(
    groupField,
    config,
    meta,
    realParentPath
...
const formatFunc = (meta, config, currentValue, parentPath) => {
  ...
  for (const argKey in funcConfig.args) {
    ...
    const fieldWidgetDef = omit(
      getFieldWidgetConfig(config, fieldDef, null, widget, argValueSrc),
      ["factory"]
    )
  ...
  if (typeof funcConfig.mongoFormatFunc === "function") {
    ...
    if (mongoArgsAsObject) ret = { [funcName]: formattedArgs }
    else if (argsCnt == 1 && lastArg !== undefined)
      ret = { [funcName]: lastArg }
    else ret = { [funcName]: Object.values(formattedArgs) }
  }

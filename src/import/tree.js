...
import { getLightTree, getTreeBadFields } from "../utils/treeUtils"
import { validateTree } from "../utils/validation"

export const getTree = (
  immutableTree,
  light = true,
  children1AsArray = true
) => {
...
function jsTreeToImmutable(tree) {
  return fromJS(tree, function (key, value) {
    ...
    if (key == "properties") {
      ...
      outValue = new Immutable.OrderedMap(
        value.map((child) => [child.get("id"), child])
      )
    } else {

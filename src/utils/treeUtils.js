...
export const removeIsLockedInTree = (tree) => {
  ...
  function _processNode(item, path, isParentLocked = false) {
    ...
    if (isParentLocked && isLocked) {
      newTree = newTree.deleteIn(
        expandTreePath(itemPath, "properties", "isLocked")
      )
    }
...
export const fixEmptyGroupsInTree = (tree) => {
  ...
  function _processNode(item, path, lev, nodeId) {
    ...
    const children = item.get("children1")
    if (children) {
      const allChildrenGone = children
        .map((child, childId) => {
...
export const getFlatTree = (tree) => {
  ...
  function _flatizeTree(
    ...
    const childrenIds = children
      ? children.map((_child, childId) => childId)
      : null
...
export const getTreeBadFields = (tree) => {
  ...
  function _processNode(item, path, lev) {
    ...
    if (
      valueError &&
      valueError.size > 0 &&
      valueError.filter((v) => v != null).size > 0
    ) {
...
export const isEmptyTree = (tree) =>
  !tree.get("children1") || tree.get("children1").size == 0

export const hasChildren = (tree, path) =>

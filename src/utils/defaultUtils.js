...
export const defaultField = (
  config,
  canGetFirst = true,
  parentRuleGroupPath = null
) => {
  return typeof config.settings.defaultField === "function"
    ? config.settings.defaultField(parentRuleGroupPath)
    : config.settings.defaultField ||
        (canGetFirst ? getFirstField(config, parentRuleGroupPath) : null)
}
...
export const defaultRuleProperties = (
  config,
  parentRuleGroupPath = null,
  item = null
) => {
  ...
  if (field && operator) {
    let { newValue, newValueSrc, newValueType, newValueError } =
      getNewValueForFieldOp(
        config,
        config,
        current,
...
export const defaultGroupConjunction = (config, fieldConfig = null) => {
  ...
  const conjs =
    (fieldConfig && fieldConfig.conjunctions) ||
    Object.keys(config.conjunctions)
  if (conjs.length == 1) return conjs[0]
  return (
    config.settings.defaultGroupConjunction ||

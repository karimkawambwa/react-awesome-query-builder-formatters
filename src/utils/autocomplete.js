...
export const simulateAsyncFetch =
  (all, cPageSize = 0, delay = 1000) =>
  async (search, offset, meta) => {
    const pageSize = meta.pageSize != undefined ? meta.pageSize : cPageSize
    const filtered = listValuesToArray(all).filter(({ title }) =>
      search == null
        ? true
...
export const mergeListValues = (values, newValues, toStart = false) => {
  ...
  const old = values || []
  const newFiltered = newValues.filter(
    (v) => old.find((av) => av.value == v.value) == undefined
  )

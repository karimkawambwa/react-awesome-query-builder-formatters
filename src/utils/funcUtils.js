...
import { filterValueSourcesForField } from "react-awesome-query-builder-formatters/dist/utils/ruleUtils"
...
export const completeFuncValue = (value, config) => {
  const _checkFuncValue = (value) => {
    ...
    for (const argKey in funcConfig.args) {
      ...
      const filteredValueSources = filterValueSourcesForField(
        config,
        valueSources,
        argConfig
      )
      const args = complValue.get("args")
      const argDefaultValueSrc =
        filteredValueSources.length == 1 ? filteredValueSources[0] : undefined
      const argVal = args ? args.get(argKey) : undefined
      const argValue = argVal ? argVal.get("value") : undefined
      const argValueSrc =
        (argVal ? argVal.get("valueSrc") : undefined) || argDefaultValueSrc
  ...
  const _traverse = (value) => {
    ...
    for (const arg of args.values()) {
      if (arg.get("valueSrc") == "field") {
        ...
        if (rightField) {
          const rightFieldDefinition = config
            ? getFieldConfig(config, rightField)
            : undefined
...
export const setFunc = (value, funcKey, config) => {
  ...
  if (funcConfig) {
    for (const argKey in funcConfig.args) {
      ...
      const firstValueSrc = filteredValueSources.length
        ? filteredValueSources[0]
        : undefined
      const defaultValueSrc = defaultValue
        ? isObject(defaultValue) && !!defaultValue.func
          ? "func"
          : "value"
        : undefined
...
export const setArgValueSrc = (
  ...
  if (value && value.get("func")) {
    value = value.setIn(
      ["args", argKey],
      new Immutable.Map({ valueSrc: argValSrc })
    )
  }
  return value
}
